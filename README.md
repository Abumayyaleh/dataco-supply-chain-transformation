...
1. Clone this repository and create a dedicated PostgreSQL database named `dataco_portfolio` using your normal local authentication.
2. Download the CSV and place it at `data/DataCoSupplyChainDataset.csv`; follow [data/README.md](data/README.md).
3. Run **section 0 only** of `01_bronze_profiling.sql` to create the text staging table, then import the CSV using the documented `psql` command.
4. Execute the scripts from the repository root in this order:

```sh
psql -X -v ON_ERROR_STOP=1 -d dataco_portfolio -f sql/01_bronze_profiling.sql
psql -X -v ON_ERROR_STOP=1 -1 -d dataco_portfolio -f sql/02_silver_transformation.sql
psql -X -v ON_ERROR_STOP=1 -1 -d dataco_portfolio -f sql/03_gold_star_schema.sql
psql -X -v ON_ERROR_STOP=1 -d dataco_portfolio -f sql/04_business_analysis.sql
```

Inspect Bronze results before proceeding to Silver, and validate Gold before interpreting analysis. Scripts 02 and 03 **drop and rebuild their own target tables**; use a dedicated project database. The `-1` option wraps each build in a transaction. Do not import the CSV twice into the same Bronze table.

### Release verification status

The publication review independently recalculated source row count, unique items, orders, sales, profit, margin, loss rate, negative-profit concentration, and late-order rate from the local CSV. These agree with the reported KPIs. Repository links, images, execution order, and public-file hygiene were checked. A fresh PostgreSQL end-to-end execution was **not completed** because local Windows application control blocked the PostgreSQL executable; the included database validation queries must still be run in an allowed PostgreSQL environment.

## Repository structure

```text
dataco-supply-chain-transformation/
├── README.md
├── .gitignore
├── sql/
│   ├── 01_bronze_profiling.sql
